...
from train_two_stage_copy_copy_copy import train_GANO_HM_two_stage_copy_copy_copy
import random
...
if __name__ == "__main__":
    ...
    with open('config/config_6.yaml', "r") as f:
    ...
    D_move = small_cDiscriminator_HM(config['move_dim_info']+config['move_dim_info']+2*config['n_feat'], config['d_co_domain'], config['n_feat'], config['modes'], pad=config['npad']).to(config['device']).float()
    D_orient = small_cDiscriminator_HM(config['orient_dim_info']+config['orient_dim_info']+2*config['n_feat'], config['d_co_domain'], config['n_feat'], config['modes'], pad=config['npad']).to(config['device']).float()
    D_betas = small_cDiscriminator_HM(config['betas_dim_info']+config['betas_dim_info']+2*config['n_feat'], config['d_co_domain'], config['n_feat'], config['modes'], pad=config['npad']).to(config['device']).float()
    D_hands = cDiscriminator_HM(config['hands_dim_info']+config['hands_dim_info']+2*config['n_feat'], config['d_co_domain'], config['n_feat'], config['modes'], pad=config['npad']).to(config['device']).float()
    D_legs = mid_cDiscriminator_HM(config['legs_dim_info']+config['legs_dim_info']+2*config['n_feat'], config['d_co_domain'], config['n_feat'], config['modes'], pad=config['npad']).to(config['device']).float()
    D_torso = mid_cDiscriminator_HM(config['torso_dim_info']+config['torso_dim_info']+2*config['n_feat'], config['d_co_domain'], config['n_feat'], config['modes'], pad=config['npad']).to(config['device']).float()

    G_move = small_cGenerator_HM(config['codim']+config['move_dim_info']+2*config['n_feat'], config['d_co_domain'], config['move_dim_info'], config['n_feat'], config['modes'], pad=config['npad']).to(config['device']).float()
    G_orient = small_cGenerator_HM(config['codim']+config['orient_dim_info']+2*config['n_feat'], config['d_co_domain'], config['orient_dim_info'], config['n_feat'], config['modes'], pad=config['npad']).to(config['device']).float()
    G_betas = small_cGenerator_HM(config['codim']+config['betas_dim_info']+2*config['n_feat'], config['d_co_domain'], config['betas_dim_info'], config['n_feat'], config['modes'], pad=config['npad']).to(config['device']).float()
    G_hands = cGenerator_HM(config['codim']+config['hands_dim_info']+2*config['n_feat'], config['d_co_domain'], config['hands_dim_info'], config['n_feat'], config['modes'], pad=config['npad']).to(config['device']).float()
    G_legs = mid_cGenerator_HM(config['codim']+config['legs_dim_info']+2*config['n_feat'], config['d_co_domain'], config['legs_dim_info'], config['n_feat'], config['modes'], pad=config['npad']).to(config['device']).float()
    G_torso = mid_cGenerator_HM(config['codim']+config['torso_dim_info']+2*config['n_feat'], config['d_co_domain'], config['torso_dim_info'], config['n_feat'], config['modes'], pad=config['npad']).to(config['device']).float()
    ...
    final_result = train_GANO_HM_two_stage_copy_copy_copy(D_move, D_orient, D_betas, D_hands, D_legs, D_torso,
                                                ...
                                                G_move_scheduler, G_orient_scheduler, G_betas_scheduler, G_hands_scheduler, G_legs_scheduler, G_torso_scheduler, data_stats=None, scheduler=None)
