...
writer = SummaryWriter("/scratch/gilbreth/anugu/logs_demo/demo_2_models_6")
...
def train_GANO_HM_two_stage(D_transl, D_pose, G_transl, G_pose, train_dataloader, val_dataloader,
    ...
    for i in tqdm(range(epochs)):
        ...
        for j, data in tqdm(enumerate(train_dataloader)):
            ...
            if (j + 1) % n_critic == 0:
                ...
                loss_common = hm_weight*human_loss(x_syn_full) + fc_weight*balance_loss(c, x_syn_full)
                loss_G1 = -torch.mean(D_transl(x_syn_transl,c_transl))*10 + loss_common + gt_weight*F.mse_loss(x_syn_transl, x_transl)
                loss_G2 = -torch.mean(D_pose(x_syn_poses,c_pose))*10+ loss_common + gt_weight*F.mse_loss(x_syn_poses, x_pose)
        ...
        if i%10==0:    
            ...
            torch.save(D_transl.state_dict(), logging['D_trans_save_path']+f"discriminator_trans_epoch{i}.pt")

            torch.save(G_pose.state_dict(), logging['G_pose_save_path']+f"generator_pose_epoch{i}.pt")
            torch.save(D_pose.state_dict(), logging['D_pose_save_path']+f"discriminator_pose_epoch{i}.pt")

            torch.save(D_transl_optimizer.state_dict(), logging['D_transl_optimizer_save_path']+f"epoch{i}.pt")
            ...
            torch.save(D_pose_optimizer.state_dict(), logging['D_pose_optimizer_save_path']+f"epoch{i}.pt")

            torch.save(torch.from_numpy(losses_D_transl), logging['plots']+f"loss_D_transl.pt")
            torch.save(torch.from_numpy(losses_G_transl), logging['plots']+f"loss_G_transl.pt")
            torch.save(torch.from_numpy(losses_W_transl), logging['plots']+f"loss_W_transl.pt")
            ...
            torch.save(torch.from_numpy(losses_D_poses), logging['plots']+f"loss_D_poses.pt")
            torch.save(torch.from_numpy(losses_G_poses), logging['plots']+f"loss_G_poses.pt")
            torch.save(torch.from_numpy(losses_W_poses), logging['plots']+f"loss_W_poses.pt")
        ...
        with open(logging['log_loss_path'], 'a') as f:
            print("Epochs ",i, "D_trans: ", losses_D_transl[i],"D_poses: ", losses_D_poses[i], "G_trans: ", losses_G_transl[i], "G_pose: ", losses_G_poses[i],"W_transl: ", losses_W_transl[i], "W_poses", losses_W_poses[i], "losses MSE", losses_MSE_full[i], "losses_V_MSE", losses_V_MSE_full[i], file=f)

        
    return losses_D_transl, losses_G_transl, losses_W_transl, losses_MSE_transl, losses_V_MSE_transl, losses_D_poses, losses_G_poses, losses_W_poses, losses_MSE_poses, losses_V_MSE_poses, losses_MSE_full, losses_V_MSE_full