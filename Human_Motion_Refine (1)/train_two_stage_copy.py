...
writer = SummaryWriter("/scratch/gilbreth/anugu/logs_demo/demo_2_models_overfit_one_batch_v4")
...
def train_GANO_HM_two_stage_copy(D_transl, D_pose, G_transl, G_pose, train_dataloader, val_dataloader,
    ...
    for i in tqdm(range(epochs)):
        ...
        for j, data in tqdm(enumerate(train_dataloader)):
            ...
            if (j + 1) % n_critic == 0:
                ...
                loss_G1 = -torch.mean(D_transl(x_syn_transl,c_transl)) + loss_common + gt_weight*F.mse_loss(x_syn_transl, x_transl)
                loss_G2 = -torch.mean(D_pose(x_syn_poses,c_pose))+ loss_common + gt_weight*F.mse_loss(x_syn_poses, x_pose)
