...
train_mean = torch.from_numpy(np.load('data/data_more_processing/train_mean_100.npy')).to('cuda:0')
train_std = torch.from_numpy(np.load('data/data_more_processing/train_std_100.npy')).to('cuda:0')

with open('config/config_new_6.yaml', "r") as f:
...
def train_all_generators(x_move, c_move, D_move, G_move, G_move_optimizer, G_move_scheduler,
    ...
    # Move generator
    G_move_optimizer.zero_grad()
    x_syn_move = G_move(grf.sample(x_move.shape[0], x_move.shape[1]).float(), c_move)
    loss_G_move_now = gt_weight * F.mse_loss(x_syn_move, x_move) - torch.mean(D_move(x_syn_move, c_move))

    # Orient generator
    G_orient_optimizer.zero_grad()
    x_syn_orient = G_orient(grf.sample(x_orient.shape[0], x_orient.shape[1]).float(), c_orient)
    loss_G_orient_now = gt_weight * F.mse_loss(x_syn_orient, x_orient) - torch.mean(D_orient(x_syn_orient, c_orient))

    # Betas generator
    G_betas_optimizer.zero_grad()
    x_syn_betas = G_betas(grf.sample(x_betas.shape[0], x_betas.shape[1]).float(), c_betas)
    loss_G_betas_now = gt_weight * F.mse_loss(x_syn_betas, x_betas) - torch.mean(D_betas(x_syn_betas, c_betas))

    # Hands generator
    G_hands_optimizer.zero_grad()
    x_syn_hands = G_hands(grf.sample(x_hands.shape[0], x_hands.shape[1]).float(), c_hands)
    loss_G_hands_now = gt_weight * F.mse_loss(x_syn_hands, x_hands) - torch.mean(D_hands(x_syn_hands, c_hands))

    # Legs generator
    G_legs_optimizer.zero_grad()
    x_syn_legs = G_legs(grf.sample(x_legs.shape[0], x_legs.shape[1]).float(), c_legs)
    loss_G_legs_now = gt_weight * F.mse_loss(x_syn_legs, x_legs) - torch.mean(D_legs(x_syn_legs, c_legs))

    # Torso generator
    G_torso_optimizer.zero_grad()
    x_syn_torso = G_torso(grf.sample(x_torso.shape[0], x_torso.shape[1]).float(), c_torso)
    loss_G_torso_now = gt_weight * F.mse_loss(x_syn_torso, x_torso) - torch.mean(D_torso(x_syn_torso, c_torso))

    # full generator
    x_syn_full = combine_data(x_syn_move, x_syn_orient, x_syn_betas, x_syn_hands, x_syn_legs, x_syn_torso)
    x = combine_data(x_move, x_orient, x_betas, x_hands, x_legs, x_torso)
    loss_human = hm_weight*human_loss(x_syn_full) + fc_weight*balance_loss(x_syn_full, x)

    # Combine losses
    total_loss = loss_G_move_now + loss_G_orient_now + loss_G_betas_now + loss_G_hands_now + loss_G_legs_now + loss_G_torso_now + loss_human
    
    # Backward on combined loss
    total_loss.backward()

    # Update all optimizers
    G_move_optimizer.step()
    G_move_scheduler.step()
    G_orient_optimizer.step()
    G_orient_scheduler.step()
    G_betas_optimizer.step()
    G_betas_scheduler.step()
    G_hands_optimizer.step()
    G_hands_scheduler.step()
    G_legs_optimizer.step()
    G_legs_scheduler.step()
    G_torso_optimizer.step()
    G_torso_scheduler.step()

    return x_syn_full, loss_G_move_now.item(), loss_G_orient_now.item(), loss_G_betas_now.item(), loss_G_hands_now.item(), loss_G_legs_now.item(), loss_G_torso_now.item()
    


def train_GANO_HM_two_stage_copy_copy_copy_copy_copy(D_move, D_orient, D_betas, D_hands, D_legs, D_torso,
                                           G_move, G_orient, G_betas, G_hands, G_legs, G_torso,
                                            ...
                                            resume_epoch, data_stats = None, scheduler=None):

    writer = SummaryWriter("/scratch/gilbreth/anugu/logs_demo/" + config['experiment_name'])
    ...
    for i in tqdm(range(int(resume_epoch), epochs)):
        ...
        if i%100==0:
            ...
            torch.save(D_move_scheduler.state_dict(), logging['D_move_scheduler_save_path']+f"epoch{i}.pt")
            torch.save(D_orient_scheduler.state_dict(), logging['D_orient_scheduler_save_path']+f"epoch{i}.pt")
            torch.save(D_betas_scheduler.state_dict(), logging['D_betas_scheduler_save_path']+f"epoch{i}.pt")
            torch.save(D_hands_scheduler.state_dict(), logging['D_hands_scheduler_save_path']+f"epoch{i}.pt")
            torch.save(D_legs_scheduler.state_dict(), logging['D_legs_scheduler_save_path']+f"epoch{i}.pt")
            torch.save(D_torso_scheduler.state_dict(), logging['D_torso_scheduler_save_path']+f"epoch{i}.pt")

            torch.save(G_move_scheduler.state_dict(), logging['G_move_scheduler_save_path']+f"epoch{i}.pt")
            torch.save(G_orient_scheduler.state_dict(), logging['G_orient_scheduler_save_path']+f"epoch{i}.pt")
            torch.save(G_betas_scheduler.state_dict(), logging['G_betas_scheduler_save_path']+f"epoch{i}.pt")
            torch.save(G_hands_scheduler.state_dict(), logging['G_hands_scheduler_save_path']+f"epoch{i}.pt")
            torch.save(G_legs_scheduler.state_dict(), logging['G_legs_scheduler_save_path']+f"epoch{i}.pt")
            torch.save(G_torso_scheduler.state_dict(), logging['G_torso_scheduler_save_path']+f"epoch{i}.pt")
